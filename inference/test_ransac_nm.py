...
for _ in bar:
    ...
    def res_func_combined(param, normal1_nm, normal2_nm):
        G21_tilde = np.array(param).reshape(3,3)

        normal2_nm_warped = normal2_nm @ G21_tilde.T
        normal2_nm_warped /= (np.linalg.norm(normal2_nm_warped, axis=1, keepdims=True) + 1e-3)
        normal1_nm_warped = normal1_nm @ np.linalg.inv(G21_tilde).T
        normal1_nm_warped /= (np.linalg.norm(normal1_nm_warped, axis=1, keepdims=True) + 1e-3)

        return np.concatenate([
            (normal1_nm - normal2_nm_warped).reshape(-1),
            (normal2_nm - normal1_nm_warped).reshape(-1),
            1e-3 * (G21_tilde[2,2] - 1).reshape(-1)
        ]).reshape(-1)

    G21_tilde_naive = scipy.optimize.least_squares(
        res_func_combined, 
        np.eye(3).reshape(-1),
        args=(
    ...
    if np.linalg.det(G21_tilde_naive) < 1e-4:
        continue
    ...
    for phi_est, theta_est in possible_phi_theta:
        for eta_est in [-0.25 * np.pi, 0.25 * np.pi]:
            ...
            decom_res = decompose_invalid_relative_gbr(G21_tilde_naive, R21_est)
            l1_est, m1_est, n1_est, l2_est, m2_est, n2_est = decom_res['params']

            if (l1_est < 1e-2) or (l2_est < 1e-2):
                continue

            if (l1_est > 4.5) or (l2_est > 4.5):
                continue

            #print(G21_tilde_naive)

            G21_tilde_naive = decom_res['G21_tilde']
            #print(G21_tilde_naive)
            #print(l1_est, m1_est, n1_est, l2_est, m2_est, n2_est)

            param_init = encode_param(l1_est, m1_est, n1_est, l2_est, m2_est, n2_est,R21_est)
    
            #resi = np.sum(res_func(
            #    param_init,
...
if use_nreg:
    ...
    print(
        '-',eval_param_nm(best_param)[0], 
        'x', eval_param_rm(best_param)[0], 
        'x', eval_param_reg(best_param), 
        ' =', res_func_(best_param)
    )
    print(phi_est, eta_est, theta_est)
    print(l1_est, m1_est, n1_est, l2_est, m2_est, n2_est)
    ...
    for _ in range(1):
        result = scipy.optimize.minimize(
            ...
            bounds=[
                ...
                (-10,10),
                (-10,10),
                #(best_param[0]-1e-6,best_param[0]+1e-6),
                #(best_param[1]-1e-6,best_param[1]+1e-6),
                #(best_param[2]-1e-6,best_param[2]+1e-6),
                #(best_param[3]-1e-6,best_param[3]+1e-6),
                #(best_param[4]-1e-6,best_param[4]+1e-6),
                #(best_param[5]-1e-6,best_param[5]+1e-6),
                (None,None),#(best_param[-3]-1e-6,best_param[-3]+1e-6),
                (None,None),#(best_param[-2]-1e-6,best_param[-2]+1e-6),
                (None,None),#(best_param[-1]-1e-6,best_param[-1]+1e-6),
            ]
        )
        
        if res_func_(result.x) < res_func_(best_param):
            best_param = result.x
            l1_est, m1_est, n1_est, l2_est, m2_est, n2_est,R21_est = decode_param(best_param)
